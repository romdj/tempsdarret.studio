...
import { User, UserRole } from '@tempsdarret/shared/schemas/user.schema';
...
// User mongoose schema based on user-service.tsp User model
...
// Indexes for query performance based on UserQuery from TypeSpec
userSchema.index({ email: 1 });
userSchema.index({ role: 1 });
userSchema.index({ isActive: 1 });
userSchema.index({ createdAt: -1 });
userSchema.index({ email: 1, role: 1 }); // Composite index for common queries
userSchema.index({ name: 'text', email: 'text' }); // Text search index for UserQuery.search
