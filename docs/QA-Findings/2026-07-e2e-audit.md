...
|----|-----|--------|---------|
| QA-1 | P1 | DONE | Notification health gate blocks E2E setup |
| QA-2 | P1 | DONE | No test isolation / cleanup |
| QA-3 | P1 | OPEN | Missing `docker-compose.test.yml` (→ Phase 8) |
| QA-4 | P1 | DONE | Stale sibling E2E files (02/03) fail the suite |
| QA-5 | P2 | DONE | Non-deterministic Kafka topic/consumer-group lifecycle |
...
> Status: QA-1/2/4/5 addressed in the E2E isolation/determinism work (topic
> pre-creation, runtime-discovered DB wipe in a global `beforeEach`, notification
> removed from the HTTP health gate, stale 02/03 quarantined). QA-3 + the CI job
> are next; QA-6/7/8 remain.
